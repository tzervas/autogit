...
This directory contains the AI agent configuration files for GitHub Copilot. The system uses a **multiagent architecture** where specialized agents collaborate to deliver high-quality code, infrastructure, and documentation.
...
2. **Let the root agent coordinate**: It will automatically delegate to specialized agents
3. **Review at checkpoints**: Ask to pause for review between major steps
4. **Explicitly invoke if needed**: Use `@agent-name` for specific agents
...
The root agent will orchestrate Project Manager → Software Engineer → DevOps → Security → Documentation → Evaluator.
...
2. Update `shared-context.md` when changing project standards
3. Test changes using the 5-test framework
4. Update individual agent files for specialized behaviors
...
2. **Load on Demand**: Specialized agents based on keywords
3. **Explicit Loading**: User can request specific agent with `@agent-name`
...
2. **Context Window**: Large requests may need multiple sessions
3. **No State Persistence**: Use commits and files to track state
4. **No Direct Agent Communication**: Root agent coordinates all
...
2. **Specialized Agents**: Domain-specific behaviors
3. **Root Agent** (`agent.md`): Coordination logic
4. **Test**: Run verification tests
5. **Document**: Update this README if structure changes
...
2. Check `TESTING-GUIDE.md` for troubleshooting
3. Review individual agent files for specific domains
...
2. Make focused changes to relevant agent file(s)
3. Update shared-context.md if changing standards
4. Test with the 5-test framework
5. Update documentation
6. Submit PR with clear explanation
