...
**Status**: Accepted **Date**: 2025-12-21 **Deciders**: Project Team **Technical Story**: Core
infrastructure selection

## Context

AutoGit requires an ingress controller to manage external access to services in the Kubernetes
cluster. The ingress controller must:
...
1. **NGINX Ingress Controller** (Apache 2.0)
1. **HAProxy Ingress** (Apache 2.0)
1. **Envoy/Contour** (Apache 2.0)
...
1. **NGINX EOL**: NGINX Ingress Controller will be retired in March 2026 with no further security
   updates
1. **Native Let's Encrypt**: Traefik has built-in ACME support without additional controllers
1. **Dynamic Configuration**: Automatic service discovery and configuration
1. **MIT License**: Perfect compatibility with project license
1. **Modern Architecture**: Built specifically for cloud-native environments
1. **Better DX**: Simpler configuration than NGINX
1. **Active Development**: Regular releases and security patches

### Positive Consequences

- No need to manage separate cert-manager for basic SSL (though we still use it for advanced
  features)
...
None (current) --**Last Updated**: 2025-12-21 **Status**: Accepted and Implemented
