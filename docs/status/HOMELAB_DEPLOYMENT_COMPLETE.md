...
Successfully deployed AutoGit infrastructure to homelab server (192.168.1.170) using rootless Docker with Terraform orchestration. Both GitLab CE and Runner Coordinator are running and healthy.
...
runner-coordinator:
  ...
  user: "0:0"
```
**Rationale**: In rootless Docker, UID 0 inside the container maps to the host user (UID 1000), allowing the coordinator to access the Docker socket.
  ...
  deploy:
    resources:
      ...
      reservations:
        ...
        memory: 16G
```
**Rationale**: Leveraging the 56-thread CPU to accelerate GitLab initialization and reduce job execution time.
...
```
**Rationale**: Ensures all Docker commands use the rootless socket instead of the system-wide `/var/run/docker.sock`.
...
1. **Port Mapping Matters**: GitLab's healthcheck must match the internal listening port (3000), not the mapped external port (80→3000).

2. **Rootless UID Mapping**: Understanding that `user: "0:0"` in rootless Docker maps to the host user (not actual root) is crucial for socket access.

3. **Resource Allocation**: Generous CPU/RAM limits significantly reduce GitLab startup time (from 10+ mins to 3-5 mins).

4. **Interactive Dashboards**: Background health checks prevent UI hangs and improve developer experience.

5. **Fast Iteration**: The `sync → up --build` flow is essential for rapid development on remote hardware.
...
**Deployment Status**: ✅ Production-Ready for AMD64 Homelab
**Next Milestone**: Automated Runner Registration and Job Execution
