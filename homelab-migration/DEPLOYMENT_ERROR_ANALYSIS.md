...
2. Normal initialization messages before services are ready
3. Generic warning messages that always appear
...
**Why this happens:**
GitLab includes templates for error pages and helper libraries for failover scenarios. These are normal files being cached during initialization.
...
2. **Puma** (Rails web server) starts later (~2-3 minutes)
3. Workhorse tries to connect to Puma's socket file
4. Socket doesn't exist yet → connection refused
5. Workhorse retries every 10 seconds
6. Once Puma creates the socket, these stop

**Why this is normal:**
This is the **exact expected behavior** during initialization. Services start in sequence, and early-starting services wait for late-starting services.
...
**Why this appears:**
GitLab's entrypoint script always prints this message as a preemptive troubleshooting tip.
...
**Why grep caught it:**
The filter was looking for "migrat" to catch database migration messages, but also caught Chef cookbook filenames.
...
2. ✅ Database migration progress (real migrations, not Chef files)
3. ✅ Service startup (Puma, Workhorse, Sidekiq)
4. ✅ Worker configuration (Puma workers, Sidekiq concurrency)
5. ✅ Health check status
6. ⚠️  Real errors (after startup phase)

### What to SUPPRESS:
1. ❌ Chef cookbook caching messages
2. ❌ Connection refused during first 90 seconds
3. ❌ Generic warning messages
4. ❌ Filenames containing "error" or "fail"
5. ❌ Info-level log messages
...
2. **Connection refused during startup = normal**
3. **INFO/WARN logs ≠ failures**
4. **Time-based filtering is essential**
5. **Focus on milestones, not noise**
...
2. Monitor with `check-status.sh` for periodic checks
3. If you see REAL errors (after 90 seconds):
...
**Bottom Line:** All "errors" during deployment were false positives. The deployment is **100% healthy** and running perfectly with ULTRA BEAST MODE configuration! 🎉
