...
1. `external_url` was set to `http://gitlab.vectorweight.com:8080`
1. GitLab's Puma tried to bind to port 8080 **inside the container**
1. Port 8080 was already in use by Docker's port mapping mechanism
1. Puma crashed with "Address already in use" error
1. Container entered crash loop → unhealthy status
1. Health checks failed because Puma never stayed running

### Why This Happened

When you specify a port in `external_url`, GitLab Puma tries to bind to that port directly. In
Docker, ports are mapped at the container level, not bound inside the container.
...
**Why:** Allocating 24 cores and 32GB RAM when only using 1 core and 3GB is wasteful. Still gave 3x
safety margin.
...
1. ✅ **Puma will bind to unix socket successfully**
1. ✅ **Container will reach healthy status**
1. ✅ **Health endpoint will respond with 200 OK**
1. ✅ **Faster initialization** (less overhead from over-allocation)
