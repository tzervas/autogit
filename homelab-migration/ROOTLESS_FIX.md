...
**Problem:** Rootless Docker cannot bind to privileged ports (80, 443) without special configuration.
...
2. Apply immediately with `sysctl -p`
3. Allow original `./deploy.sh` to work with ports 80/443
...
✅ `docker-compose.rootless.yml` - Config with ports 8080/8443/2222
✅ `deploy-rootless.sh` - Deployment script for rootless Docker
✅ `fix-rootless-ports.sh` - System-wide fix for Option 1
