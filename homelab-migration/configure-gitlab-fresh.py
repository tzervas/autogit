...
class GitLabConfigurator:
    ...
    def create_group(self, name: str, path: str, description: str = "") -> Dict:
        """Create a new group"""
        data = {"name": name, "path": path, "description": description, "visibility": "private"}
    ...
    def create_project(
        self, name: str, namespace_id: int, description: str = "", visibility: str = "private"
    ) -> Dict:
...
def main():
    ...
    groups["projects"] = config.create_group(
        name="Projects", path="projects", description="Mirrored repositories from GitHub and GitLab"
    )
    print(f"  ✅ Created group: {groups['projects']['name']}")

    # Add users to groups
    print("🔗 Adding users to groups...")
    config.add_user_to_group(
        users["ci_user"]["id"], groups["projects"]["id"], access_level=40
    )  # Maintainer
    config.add_user_to_group(
        users["admin_user"]["id"], groups["projects"]["id"], access_level=50
    )  # Owner
    ...
    config_data = {
        ...
        "users": {
            k: {"id": v["id"], "username": v["username"], "email": v["email"]}
            for k, v in users.items()
        },
        "groups": {
            k: {"id": v["id"], "name": v["name"], "path": v["path"]} for k, v in groups.items()
        },
        "tokens": {
            k: {"token": v["token"], "name": v["name"], "scopes": v["scopes"]}
