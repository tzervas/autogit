...
class JobManager:
    ...
    async def process_queue(self):
        ...
        while True:
            try:
                # 1. Get queued jobs
                queued_jobs = self.db.query(Job).filter(Job.status == "queued").order_by(Job.created_at).all()
    ...
    async def cleanup_runners(self):
        ...
        for runner in busy_runners:
            ...
            if status in ["exited", "not_found"]:
                ...
                job = self.db.query(Job).filter(Job.runner_id == runner.id, Job.status == "running").first()
                if job:
